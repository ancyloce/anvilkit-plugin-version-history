...
const valueArb = fc.oneof(
	...
	fc.dictionary(
		...
		fc.array(scalarValueArb, { maxLength: 2 }),
		{
			maxKeys: 2,
		},
	),
...
const pairPlanArb = fc.record({
	...
	metaMutations: fc.array(
		fc.record({
			...
			keySeed: fc.nat(10_000),
			mode: fc.constantFrom("set", "delete"),
			value: fc.oneof(fc.boolean(), fc.string({ maxLength: 16 }), semverArb),
		}),
		{ maxLength: 4 },
	),
	adds: fc.array(
...
function buildIR(spec: {
	...
	const slots: Array<{ node: MutableNode; depth: number }> = [
		{ node: root, depth: 0 },
	];
...
function mutateIR(
	...
	for (const move of plan.moves) {
		...
		const blocked = new Set(
			collectNodes(target.node).map((entry) => entry.node),
		);
		const maxDepth = subtreeHeight(target.node);
		const destinations = collectNodes(root).filter(
			(entry) => !blocked.has(entry.node) && entry.depth + maxDepth <= 4,
		);
		const destination =
			destinations[move.parentSeed % destinations.length]?.node;
		if (!destination) {
			continue;
		}
